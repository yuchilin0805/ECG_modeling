...
class datapreprocess_ptb:
    ...
    def gen_reference_csv(self,data_dir, reference_csv, df):
        ...
        """
        results=[]
        recordpath = "/media/user/nvme0/ECG-modeling/cpsc_2018/g1/A0001.hea"
        patient_id = os.path.basename(recordpath)[:-4]
        _, meta_data = wfdb.rdsamp(recordpath[:-4])
        sample_rate = meta_data['fs']
        signal_len = meta_data['sig_len']
...
class datapreprocess_shaoxing:
    ...
    def gen_reference_csv(self,data_dir, reference_csv):
        ...
        # df=pd.read_csv('/media/user/nvme0/ECG-modeling/shaoxing-ninbo/physionet.org/files/ecg-arrhythmia/1.0.0/ConditionNames_SNOMED-CT.csv')
        # for _,row in df.iterrows():
        #     print(row)
        
        #     exit()

        # recordpath='/media/user/nvme0/ECG-modeling/shaoxing-ninbo/datas/JS23074.hea'
        # print(recordpath[:-4])
        # _, meta_data = wfdb.rdsamp(recordpath[:-4])
        # print(meta_data)
        # exit()
